...
export function ProfilePage(): React.JSX.Element {
  ...
  const displayRole =
    roleCodes.map((c) => ROLE_LABEL_MAP[c] ?? c).join(' / ') || '-';
  const roleTagColor =
    roleCodes.length > 0
      ? ROLE_TAG_COLOR_MAP[roleCodes[0]] || 'blue'
      : 'default';
  const isSuperAdmin = roleCodes.includes('super_admin');

  const permissionGroups = useMemo(
    () => buildPermissionGroups(user?.menus ?? []),
    [user?.menus],
  );
  const buttonPermissions = useMemo(() => {
    const menuCodes = new Set(
      (user?.menus ?? []).flatMap((n) => [
        n.code,
    ...
    >
      ...
      >
        <Title level={5} style={{ margin: 0 }}>
          ...
          <Text
            type="secondary"
            style={{ fontSize: 13, fontWeight: 'normal', marginLeft: 8 }}
          >
            查看当前账号信息与权限
          </Text>
        </Title>
            ...
            <Card title="账号信息">
              <Descriptions
                ...
                items={[
                  ...
                  { key: 'email', label: '邮箱', children: email || '未设置' },
                  {
                    key: 'role',
                    label: '角色名称',
                    span: 2,
