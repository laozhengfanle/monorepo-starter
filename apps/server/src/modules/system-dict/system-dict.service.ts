...
export class SysDictService {
  ...
  /** 创建字典类型 */
  async createType(input: unknown): Promise<SysDictType> {
    const data = input as CreateDictTypeInput;
    try {
  ...
  /** 更新字典类型 */
  async updateType(id: string, input: unknown): Promise<SysDictType> {
    const data = input as UpdateDictTypeInput;
  ...
  /** 删除字典类型（级联删除其 items） */
  async removeType(id: string): Promise<{ success: true }> {
    const existing = await this.prisma.client.sysDictType.findUnique({ where: { id } });
    if (!existing) {
      throw new BizException({ code: 'DICT_TYPE_NOT_FOUND', message: '字典类型不存在' });
    }
  ...
  /** 创建字典项 */
  async createItem(input: unknown): Promise<SysDictItem> {
    const data = input as CreateDictItemInput;
  ...
  /** 更新字典项 */
  async updateItem(id: string, input: unknown): Promise<SysDictItem> {
    const data = input as UpdateDictItemInput;
  ...
  /** 删除字典项 */
  async removeItem(id: string): Promise<{ success: true }> {
    const existing = await this.prisma.client.sysDictItem.findUnique({ where: { id } });
    if (!existing) {
      throw new BizException({ code: 'DICT_ITEM_NOT_FOUND', message: '字典项不存在' });
    }
