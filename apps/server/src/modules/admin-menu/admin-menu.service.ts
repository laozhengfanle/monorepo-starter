...
export class AdminMenuService {
  ...
  async update(
    ...
    const existing = await this.prisma.client.adminMenu.findUnique({
      where: { id },
    });
    if (!existing) {
    ...
    const row = await this.prisma.client.adminMenu.update({
      ...
      data: {
        ...
        ...(data.path !== undefined
          ? { path: data.type === 'menu' ? data.path : null }
          : {}),
  ...
  async remove(id: string, operatorId: string): Promise<AdminMenuNode> {
    ...
    const childCount = await this.prisma.client.adminMenu.count({
      where: { parentId: id },
    });
    if (childCount > 0) {
      throw new BizException({
  ...
  private async validateParent(
    ...
    if (!parentId) return;
    const parent = await this.prisma.client.adminMenu.findUnique({
      where: { id: parentId },
    });
    if (!parent) {
      throw new BizException({
    ...
    const valid =
      type === 'button' ? parent.type === 'menu' : parent.type === 'directory';
    if (!valid) {
      throw new BizException({
        code: 'MENU_PARENT_INVALID',
        message:
          type === 'button' ? '按钮只能挂在菜单下' : '菜单/目录只能挂在目录下',
      });
    }
  }
  ...
  private async ensureNotDescendant(
    ...
    while (cursor) {
      if (cursor === id) {
        throw new BizException({
          code: 'MENU_PARENT_DESCENDANT',
          message: '不能将菜单挂到自己的子菜单下',
        });
      }
