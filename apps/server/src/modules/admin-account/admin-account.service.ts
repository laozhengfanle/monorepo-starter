...
export class AdminAccountService {
  ...
  async list(query: {
    ...
    if (query.username) {
      where.identities = {
        some: { identityType: 'username', identifier: { contains: query.username, mode: 'insensitive' } },
      };
    }
    if (query.email) {
      where.adminProfile = { email: { contains: query.email, mode: 'insensitive' } };
    }
  ...
  async create(input: CreateAdminAccountInput, operatorId: string): Promise<AdminAccount> {
  ...
  async update(id: string, input: UpdateAdminAccountInput, operatorId: string): Promise<AdminAccount> {
    ...
    const nextEnabled = data.enabled !== undefined ? data.enabled : account.enabled;
    const nextRoleCodes = data.roleCodes !== undefined ? data.roleCodes : currentRoleCodes;
  ...
  async restore(id: string, operatorId: string): Promise<AdminAccount> {
    ...
    const recorded = (deletedAudit?.detail as { roleIds?: unknown } | null)?.roleIds;
    ...
    await this.prisma.rawClient.$transaction(async (tx) => {
      ...
      if (roleIds.length > 0) {
        ...
        if (roles.length > 0) {
          await tx.adminAccountRole.deleteMany({ where: { accountId: id } });
          await tx.adminAccountRole.createMany({
            data: roles.map((role) => ({ id: newId(), accountId: id, roleId: role.id })),
          });
        }
      } else {
        // 未记录（本功能上线前删除的历史数据无法考证原角色）：至少恢复 super_admin 角色，
        // 避免曾为超管的账户恢复后无任何权限导致系统不可用
        const superRole = await tx.adminRole.findUnique({ where: { code: 'super_admin' } });
        if (superRole) {
          await tx.adminAccountRole.deleteMany({ where: { accountId: id } });
  ...
  private async assertSuperAdminRemains(accountId: string): Promise<void> {
    ...
    if (otherCount === 0) {
      throw new BizException({ code: 'SUPER_ADMIN_PROTECTED', message: '系统至少保留一个超级管理员' });
    }
  }
